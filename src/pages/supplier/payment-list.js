...
export default function CustomerPaymentList({user}) {
    const headers = {
    ...
    useEffect(() => {
        axios.post(
            `${process.env.API_URL}/supplier/payments`, {},
            headers
        ).then(res => {
            if (res.data.status === true) {
    ...
    const deleteHandler = async (id) => {
        ...
        try {
            const response = await axios.post(`${process.env.API_URL}/supplier/payment/delete`, {
                id: id,
            }, headers);
    ...
    return (
        <>
            ...
            <ToastContainer/>
            <Layout user={user} title={`Manage Supplier Payments`}>
                <div className={`content ${mode === 'dark' ? 'dark-mode-bg-body dark' : 'body-bg'}`}>
                    <div className="custom-card">
                        <div className="row mb-4">
                            <div className="col-md-12">
                                <Link href={`/supplier/payment`} className={`btn btn-success`}>
                                    <i className="fa-solid fa-plus"/> Add Supplier Payment
                                </Link>
                            </div>
                        </div>
                        <hr/>
