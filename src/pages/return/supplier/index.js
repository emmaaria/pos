...
export default function SupplierReturnList({user}) {
    const headers = {
    ...
    useEffect(() => {
        axios.get(
            `${process.env.API_URL}/return/supplier`,
            headers
        ).then(res => {
    ...
    const search = async () => {
        ...
        setTimer(
            setTimeout(() => {
                ...
                axios.get(
                    `${process.env.API_URL}/return/supplier?name=${name}`,
                    headers
                ).then(res => {
                    if (res.data.status === true) {
                        setData(res.data.returns.data);
    ...
    const deleteHandler = async (id) => {
        ...
        try {
            const response = await axios.post(`${process.env.API_URL}/return/supplier/delete`, {
                id: id,
            }, headers);
    ...
    const getReturn = async (id) => {
        ...
        try {
            const response = await axios.get(`${process.env.API_URL}/return/supplier/${id}`, headers);
            if (response.data.status === true) {
    ...
    return (
        <>
            ...
            <ToastContainer/>
            <Layout user={user} title={`Supplier Return List`}>
                                    ...
                                    <div className="card-body p-0">
                                        <div className="row">
                                            <div className="col-md-12">
                                                <p className="mb-1">Supplier Name: {returnData?.supplierName}</p>
