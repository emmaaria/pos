...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,iron_session_next__WEBPACK_IMPORTED_MODULE_2__/* .withIronSessionApiRoute */ .n)(async (req, res)=>{
    if (req.session.user) {
        ...
        if (name && name !== "") {
            const total = await _models_Product__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({
                name: new RegExp(name, "i")
            }).count();
            const products = await _models_Product__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({
                name: new RegExp(name, "i")
            }, "name purchasePrice defaultUnitPrice").lean();
            ...
            const total = await _models_Product__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({}).count();
            const products = await _models_Product__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({}, "name purchasePrice defaultUnitPrice").skip(50 * page).limit(50);
            const totalPagesCount = Math.ceil(total / 50);
            let totalPages = [];
            for(let i = 0; i <= totalPagesCount - 1; i++){
                totalPages.push(i);
...
var __webpack_exports__ = __webpack_require__.X(0, [8534,2606], () => (__webpack_exec__(2389)));
