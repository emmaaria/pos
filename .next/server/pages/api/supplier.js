...
/* harmony import */ var _models_Supplier__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(5020);
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,iron_session_next__WEBPACK_IMPORTED_MODULE_2__/* .withIronSessionApiRoute */ .n)(async (req, res)=>{
    if (req.session.user) {
        ...
        if (name && name !== "") {
            const total = await _models_Supplier__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({
                $or: [
                    {
            ...
            const suppliers = await _models_Supplier__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({
                $or: [
                    {
                        name: new RegExp(name, "i")
                    },
            ...
            const total = await _models_Supplier__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({}).count();
            const suppliers = await _models_Supplier__WEBPACK_IMPORTED_MODULE_3__/* ["default"].find */ .Z.find({}, "name mobile address transactions").skip(50 * page).limit(50);
            const totalPagesCount = Math.ceil(total / 50);
            let totalPages = [];
            for(let i = 0; i <= totalPagesCount - 1; i++){
                totalPages.push(i);
...
var __webpack_exports__ = __webpack_require__.X(0, [8534,5351], () => (__webpack_exec__(6498)));
